<x-app-layout>
    ...
    <div class="py-6 max-w-2xl mx-auto">
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('manager.update', $evaluasi) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="user_id" id="user_id" 
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $evaluasi->user_id) == $user->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_tim" id="nama_tim" 
                        value="{{ old('nama_tim', $evaluasi->nama_tim) }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                        required>
                    @error('nama_tim')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="efektivitas_sistem" id="efektivitas_sistem" 
                            value="{{ old('efektivitas_sistem', $evaluasi->efektivitas_sistem) }}"
                            min="1" max="10"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            required>
                    ...
                    <div>
                        ...
                        <input type="number" name="produktivitas_tim" id="produktivitas_tim" 
                            value="{{ old('produktivitas_tim', $evaluasi->produktivitas_tim) }}"
                            min="1" max="10"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            required>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="catatan" id="catatan" 
                        rows="4"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('catatan', $evaluasi->catatan) }}</textarea>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
